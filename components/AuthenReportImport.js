...
const statuses={pending:'รอตัวเชื่อม LAN ตรวจสอบ',matched:'จับคู่ได้ รอตรวจรับรอง',already_present:'เลขตรงกับ HOSxP แล้ว',blocked:'ไม่ผ่านตรวจสอบ',unmatched:'ไม่พบ FIT วันที่บริการตรงกัน',ambiguous:'พบหลายรายการ',failed:'ตรวจไม่สำเร็จ'};
const reasons={invalid_cid:'เลขบัตรไม่ถูกต้อง',other_provider:'ต่างหน่วยบริการ',invalid_date:'วันที่ไม่ถูกต้อง',invalid_code:'เลข Authen ไม่ถูกต้อง',invalid_service:'ไม่มีรหัสบริการ',cancelled:'มีข้อมูลยกเลิก',unknown_status:'ไม่รู้จักสถานะ',duplicate_code:'เลข Authen ซ้ำในไฟล์',multiple_visits:'มีหลาย visit',no_visit:'ไม่พบผล FIT ที่นำเข้าและมีวันที่บริการตรงกัน',hn_conflict:'HN ไม่ตรง',name_conflict:'ชื่อไม่ตรง',not_fit:'ไม่ใช่ FIT ที่นำเข้าจากระบบนี้',multiple_insurance_codes:'มีเลขเดิมหลายเลข',existing_code_conflict:'เลขเดิมต่างกัน',code_used_elsewhere:'เลขถูกใช้กับ VN อื่น',multiple_report_rows_for_visit:'หลายแถวตรงกับ VN เดียวกัน'};
export default function AuthenReportImport(){
 ...
 return <section id="authen-report" className="card mb-4" aria-label="นำเข้ารายงาน Authen"><div className="card-body">
  ...
  <p>เลือกไฟล์รายงาน .xlsx จาก New Authen เพื่อจับคู่กับผล FIT ที่นำเข้าจากระบบนี้ โดยยึดเลขบัตรประชาชน วันที่บริการ และเลข Authen เดิมผ่าน LAN พร้อมแสดงประเภทบริการให้ตรวจสอบ</p>
  ...
  {report&&<><p>สถานะ: {report.state==='matched'?'ตรวจจาก LAN แล้ว':statuses[report.state]} {report.checked_at&&`(${new Date(report.checked_at).toLocaleString('th-TH')})`}</p>
   ...
   <div className="table-responsive"><table className="table table-sm align-middle"><thead><tr><th>เลือก</th><th>ผู้รับบริการ / เลขบัตรท้าย 4</th><th>วันที่ / บริการตามรายงาน</th><th>Authen จากรายงาน</th><th>ผล FIT / VN / เลขเดิม</th><th>ผลตรวจ</th></tr></thead><tbody>{report.rows.map(r=>{const w=report.writes?.find(w=>w.row===r.row);const m=results.get(r.row),approved=report.approved_rows.includes(r.row),eligible=['matched','already_present'].includes(m?.status);return <tr key={r.row}><td><input type="checkbox" aria-label={`เลือกแถว ${r.row}`} disabled={busy||!eligible||approved} checked={selected.includes(r.row)||approved} onChange={e=>setSelected(s=>e.target.checked?[...s,r.row]:s.filter(n=>n!==r.row))}/></td><td>{r.name}<br/><small>••••{r.cid.slice(-4)}</small></td><td>{r.serviceDate||'วันที่ไม่ถูกต้อง'}<br/>{r.serviceCode}<br/><small>{r.serviceName}</small></td><td>{r.code}</td><td>{m?.fitResult&&<div>FIT: {m.fitResult}<br/>วันที่ตรวจ: {m.fitScreenDate}</div>}{m?.vn||'-'}<br/><small>{m?.existingCode||'ไม่มีเลขเดิม'}</small></td><td>{w?({pending:'รอบันทึกผ่าน LAN',written:'บันทึก HOSxP สำเร็จ',already_present:'ตรวจแล้ว เลขตรงกับ HOSxP',blocked:'ระงับการบันทึก ต้องตรวจใหม่'}[w.state]):approved?'อนุมัติแล้ว':statuses[m?.status||'pending']}{w?.error_code&&<small className="d-block">{w.error_code}</small>}<br/><small>{(m?.reasons||r.issues).map(k=>reasons[k]||k).join(', ')}</small>{m?.warnings?.includes('name_conflict')&&<div className="text-warning-emphasis small">เลขบัตรตรง แต่ชื่อต่าง: HOSxP {m.name} — ตรวจรับรองก่อนอนุมัติ</div>}{r.statusUnconfirmed&&<div className="text-warning-emphasis small">รายงานไม่ระบุสถานะใช้งาน</div>}</td></tr>;})}</tbody></table></div>
   <label className="d-flex gap-2 my-3"><input type="checkbox" checked={confirmed} disabled={busy} onChange={e=>setConfirmed(e.target.checked)}/><span>ตรวจผล FIT บุคคล (รวมชื่อที่ต่างกัน) ประเภทบริการและสถานะปัจจุบันของรายการที่เลือกแล้ว โดยเฉพาะรายการที่รายงานไม่ระบุสถานะใช้งาน</span></label>

...
export function AuthProvider({ children }) {
  ...
  const loginWithLine = useCallback(async (lineId, pictureUrl) => {
    const { data: users, error } = await supabase
      .from('app_users')
      .select('*')
    ...
    // Use the current LINE picture in this session, even if persisting it fails.
    const avatarUrl = typeof pictureUrl === 'string' ? pictureUrl.trim() : (u.avatar_url || '');
    if (typeof pictureUrl === 'string' && avatarUrl !== (u.avatar_url || '')) {
      try {
        const { error: avatarError } = await supabase.from('app_users')
          .update({ avatar_url: avatarUrl || null }).eq('id', u.id);
        if (avatarError) console.warn('Unable to save the latest LINE profile picture.');
      } catch {
        console.warn('Unable to save the latest LINE profile picture.');
      }
    }
    ...
    const session = { 
      ...
      role: u.role, moo: u.moo || '', userId: u.id, avatarUrl 
    };

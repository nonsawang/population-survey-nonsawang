...
export default function SurveyWorkPanel({ user, refreshKey, opening, onOpenHouse }) {
  ...
  return <section className="card border-0 shadow-sm mb-4" style={{ borderRadius: 16 }} aria-labelledby="survey-work-title">
    <div className="card-body p-4">
      ...
      {busy ? <p role="status">กำลังโหลดงานสำรวจ...</p> : error ? <div role="alert" className="alert alert-danger">{error} <button className="btn btn-sm btn-outline-danger" onClick={() => setRetry(v => v + 1)}>ลองอีกครั้ง</button></div> : !moos.length ? <p role="status">ยังไม่มีหมู่บ้านที่ได้รับสิทธิ์ กรุณาติดต่อผู้ดูแล</p> : <>
        ...
        {!houses.length ? <p role="status" className="text-center bg-light rounded p-3">ไม่พบงานตามตัวกรองที่เลือก</p> : <>
          <div className="table-responsive"><table className="table align-middle"><caption>พบ {houses.length.toLocaleString('th-TH')} หลัง • เรียงงานคงค้างมากก่อน</caption><thead><tr><th scope="col">บ้าน</th><th scope="col">อสม.</th><th scope="col">สำรวจแล้ว / ทั้งหมด</th><th scope="col">คงค้าง</th><th scope="col">สถานะ</th><th scope="col">ดำเนินการ</th></tr></thead><tbody>{houses.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE).map(h => <tr key={h.key}><td className="text-nowrap">ม.{h.moo || '-'} บ้าน {h.house || 'ไม่ระบุ'}</td><td>{h.owners.join(', ')}</td><td>{h.done} / {h.total}</td><td className={h.pending ? 'text-danger fw-bold' : ''}>{h.pending} คน</td><td><span className={`badge ${h.status === 'complete' ? 'bg-success' : h.status === 'partial' ? 'bg-warning text-dark' : 'bg-secondary'}`}>{labels[h.status]}</span></td><td><button className="btn btn-sm btn-outline-primary text-nowrap" disabled={opening || !h.house || h.house === '-'} onClick={() => onOpenHouse(h.moo, h.house)} aria-label={`เปิดบ้าน ${h.house} หมู่ ${h.moo}`}>เปิดบ้าน</button></td></tr>)}</tbody></table></div>

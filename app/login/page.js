...
function LoginContent() {
  ...
  const checkLock = useRef(false); // 🟢 ตัวล็อคป้องกันระบบทำงานซ้ำซ้อน
  ...
  useEffect(() => {
    const handleLineAuth = async () => {
      // 🟢 ถ้ามี auto_login และยังไม่เคยล็อคการเช็ก
      if (autoLogin === 'true' && lineId && !user && !checkLock.current) {
        checkLock.current = true; // ล็อคทันทีไม่ให้ทำซ้ำ
        setSubmitting(true);
        setSuccessMsg('กำลังตรวจสอบสิทธิ์การเข้าใช้งาน...');
        
        console.log("👉 1. เริ่มตรวจสอบ LINE ID อัตโนมัติ:", lineId);

        try {
          const res = await loginWithLine(lineId);
          console.log("👉 2. ผลลัพธ์จาก Supabase:", res);

          if (res && res.success) {
            console.log("👉 3. มีบัญชีอยู่แล้ว กำลังพาไปหน้าหลัก");
            router.push('/');
          } else {
            console.log("👉 3. ไม่พบบัญชี กำลังพาไปหน้าผูกบัตร ปชช.");
            setSubmitting(false);
            setSuccessMsg('');
            // เปลี่ยนหน้าไปยังโหมดผูกบัญชี
          ...
          console.error("❌ 4. เกิดข้อผิดพลาดร้ายแรง (แคลช):", err);
          setError('เกิดข้อผิดพลาดในการเชื่อมต่อฐานข้อมูล กรุณาแจ้งแอดมิน');
          setSubmitting(false);
          setSuccessMsg('');
        }
      }
    ...
    handleLineAuth();
  }, [autoLogin, lineId, user, loginWithLine, router, lineName, pictureUrl]);

  const handleLogin = async () => {
    let finalUser = '', finalPass = '';
    if (loginMode === 'vhv') {
      const cleanCid = cid.replace(/\D/g, '');
      if (cleanCid.length !== 13) { setError('กรุณากรอกเลขบัตร 13 หลัก'); setShake(true); return; }
      finalUser = 'vhv' + cleanCid.slice(-6); finalPass = cleanCid;
    } else {
      if (!username || !password) { setError('กรุณากรอกข้อมูลให้ครบ'); setShake(true); return; }
      finalUser = username.trim().toLowerCase(); finalPass = password;
    }

    setSubmitting(true); setError('');
    console.log("👉 กดปุ่มผูกบัญชี User:", finalUser);
    const res = await login(finalUser, finalPass);
    
    if (res.success) {
      if (linkLineId) {
        console.log("👉 กำลังผูก LINE ID เข้าฐานข้อมูล");
        await supabase.from('app_users').update({ line_user_id: linkLineId, avatar_url: pictureUrl }).eq('username', finalUser);
      }
      router.push('/');
    } else {
      setSubmitting(false);
      setError(loginMode === 'vhv' ? 'ไม่พบข้อมูล อสม. ในระบบ หรือรหัสไม่ตรง' : res.error);
      setShake(true);
    }
  };
  ...
  return (
    <div className="login-bg">
      <div className={`login-card ${shake ? 'shake' : ''}`} style={{maxWidth: '420px', margin: '0 auto'}}>
        ...
        {error && <div className="alert alert-danger small mb-3 border-0 shadow-sm" style={{borderRadius: 12}}><i className="fa-solid fa-circle-exmark me-2"/>{error}</div>}
          ...
          <div className="fade-in">
            ...
            {loginMode === 'vhv' ? (
              <div className="mb-4">
                ...
                <input type="text" maxLength="13" className="form-control form-control-lg text-center shadow-sm" placeholder="0-0000-00000-00-0" value={cid} onChange={e => setCid(e.target.value.replace(/\D/g, ''))} disabled={submitting} style={{borderRadius: 12, letterSpacing: '2px', fontWeight: 'bold'}} />
              </div>
            ) : (
              <div className="mb-4">
                <div className="mb-3">
                  <label className="form-label small fw-bold text-secondary">ชื่อผู้ใช้งาน</label>
                  <input type="text" className="form-control shadow-sm" placeholder="Username" value={username} onChange={e => setUsername(e.target.value)} disabled={submitting} style={{borderRadius: 12}} />
                </div>
                <div>
            ...
            <button className="btn btn-link w-100 mt-3 text-decoration-none text-muted small" onClick={() => router.replace('/login')} disabled={submitting}>

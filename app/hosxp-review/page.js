...
export default function HosxpReviewPage() {
  ...
  if (!user) return null;
  return <><TopBar showAdmin /><main className="container survey-page py-4">{allowed && <><PersonIdentity key={identity+':identity'}/><AuthenReportImport key={identity}/><ScreeningReview/></>}

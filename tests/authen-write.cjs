...
const visit={vn:'690921000001',hn:'0000001',cid:row.cid,name:row.name,service_date:row.serviceDate,authCodes:[]};
...
(async()=>{
 ...
 for(const options of [{old:'OTHER'},{many:true},{name:'Other'},{engine:'MyISAM'},{readbackFail:true}]){t=database(options);await assert.rejects(()=>write(t.db,job,batch));assert.equal(t.state.commits,0);assert.equal(t.state.old,options.old||'');}

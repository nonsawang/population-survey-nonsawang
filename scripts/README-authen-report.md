...
FIT matching policy (2026-09-24): Only visits linked by survey_fit_import_ledger to an imported FIT preparation and its matching live lab result are candidates. CID is primary; report service date must match the live visit date. The original FIT screening date is displayed separately. Name differences are warnings requiring staff review, while HN conflicts, ambiguous candidates, reused codes and conflicting existing codes block writes. Approval snapshots include the preparation ID, result, screening date and lab number; the writer checks these again and rejects legacy snapshots. Existing unapproved reports must be rechecked after deployment. No clinical dates are changed.

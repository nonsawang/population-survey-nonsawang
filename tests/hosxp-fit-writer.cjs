...
function fake({failTable,mismatch=false,collision=false,duplicatePP=false,missingPP=false}={}){
 ...
 async execute(sql,args=[]){
  ...
  if(sql.startsWith('SELECT pp_special_type_id'))return [missingPP?[]:[{pp_special_type_id:args[0]==='1B0060'?238:239}]];
  if(sql.startsWith('SELECT p.pp_special_id'))return [duplicatePP?[{pp_special_id:1}]:[]];
  if(sql.startsWith('SELECT pp_special_code'))return [current.pp_special||[]];
...
(async()=>{
 ...
 assert.equal(db.state.ovst[0].pt_subtype,1);assert.equal(db.state.ovst_seq[0].promote_visit,'N');
 assert.equal(db.state.pp_special[0].pp_special_code,'1B0060');
 assert.equal(db.state.pp_special[0].entry_datetime,'2026-09-10 11:22:33');
 const positive=fake();await writeVisit(positive,{...context,job:{...job,payload:{lab_result:'Positive'}}});
 assert.equal(positive.state.pp_special[0].pp_special_type_id,239);assert.equal(positive.state.pp_special[0].pp_special_code,'1B0061');
 ...
 for(const opts of [{failTable:'lab_order'},{failTable:'pp_special'},{duplicatePP:true},{missingPP:true},{mismatch:true},{collision:true}]){

...
function matchRows(rows,visits){
 const results=rows.map(r=>{
  ...
  const matches=visits.filter(v=>text(v.cid)===r.cid&&v.service_date===r.serviceDate&&v.fitPreparationId&&v.fitResult);
  ...
  const v=matches[0],reasons=[],warnings=[];
  ...
  if(r.name.replace(/\s/g,'')!==text(v.name).replace(/\s/g,''))warnings.push('name_conflict');
  ...
  const snapshot={vn:v.vn,hn:v.hn,name:v.name,serviceDate:v.service_date,existingCode:v.authCodes[0]||'',serviceCode:r.serviceCode,fitPreparationId:v.fitPreparationId,fitScreenDate:v.fitScreenDate,fitResult:v.fitResult,labOrderNumber:v.labOrderNumber};
  return {...base,...snapshot,warnings,status:reasons.length?'blocked':v.authCodes.includes(r.code)?'already_present':'matched',reasons,
